...
### CPU Load ###
	...
	}  elsif($load_1m > $warn and $load_1m < $crit) {
		$stat = 1;
	...
	$perf = "cpu_5s=$load_5s\percent; cpu_1m=$load_1m\percent; cpu_5m=$load_5m\percent";
	#$stat = 0;
	#$msg = "Cpu: OK - Cpu Load $load_1m% $load_5m%";
	#$perf = "cpu_1m=$load_1m\percent cpu_5m=$load_5m\percent";
